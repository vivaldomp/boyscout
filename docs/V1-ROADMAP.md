...
| **SP8** | **Full E2E, Skill, Lockfile & Hardening** | `skill-template`, Playwright E2E, `boyscout.lock` (transitive closure), matured cross-OS golden | all | Full agent → CLI → browser → approval → generate green in CI; lockfile reproducibility verified; §21 checklist met — ***✅ DONE**, shipped as two slices: **SP8a** Skill Composition (`skill-template` + Bridge Skill fragments, PR #11) and **SP8b** E2E/Lockfile/Hardening (`@boyscout/lockfile` + `--check` drift gate, ubuntu-only Playwright E2E, matured cross-OS golden, §21 checklist gate, PR #12); CI green on macOS/Ubuntu/Windows. Lockfile pins bridge-version + capability names (registry surfaces no per-capability contracts).* |
